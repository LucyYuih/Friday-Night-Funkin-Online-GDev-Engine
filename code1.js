...
(function () {
  ...
  async function decryptPayload(password, fileText){
    ...
    if (!equalConstTime(expected, hmac)) throw new Error("Falha de integridade/HMAC (senha errada ou arquivo modificado).");
  ...
  async function openExportUI(runtimeScene){
    ...
    btnExport.addEventListener('click', async ()=>{
      ...
      try {
        ...
        const outText = await encryptPayload(secret, json);
  ...
  async function openImportUI(runtimeScene){
    ...
    btnImport.addEventListener('click', async ()=>{
      ...
      try {
        ...
        const secret = username + (token ? ('|' + token) : '');
        // decrypt
        const plain = await decryptPayload(secret, chosenFileText);
